...
void writeReg(uint8_t regAddress, int8_t val)
{
    ...
    // Configure SPI for write operation
    ...
    //spi->txData(1,&buf[0]);
   	...
   	//nrf_delay_us(100);
		...
		//APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, &buf[1], 1, m_rx_buf, 1));
...
int8_t readReg(uint8_t regAddress)
{
	...
	#if 0
    // Configure SPI for write operation
    spi->configure(500000, SpiInterface::SPI_MSB_FIRST,
                      SpiInterface::SPI_SS_ACTIVE_LOW, SpiInterface::SPI_MODE_3);
	#endif
    ...
    //spi->exchangeData(2, txbuf, rxbuf);
    val=rxbuf[1];

    return val;
}

void readRegs(uint8_t regAddress, uint8_t *motionE, uint16_t *motionX, uint16_t *motionY)
{
	uint8_t val;
	uint8_t txbuf[4]={0,0xFF,0xFF,0xFF};
	uint8_t rxbuf[4];
	
  txbuf[0]=regAddress;
	APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, txbuf, 4, rxbuf, 4));
...
{	
  ...
  //int8_t x;
	...
	*y=0;
	...
	if (deviceFound)
	{
		//while (maxReads--&&!gpio->getPinInput(Paw3205MOTION_PORT,Paw3205MOTION_PIN))
		//while (!gpio->getPinInput(Paw3205MOTION_PORT,Paw3205MOTION_PIN))
		//{
			//Read motion value!
		
			//MotionE=readReg(0x02);
			//NRF_LOG_INFO("MotionE=%02x\r\n",MotionE);
			//Motion occurred and  xy No overflow
			
			//while((maxReads--)&&(readReg(0x02)&0x80))//&&(!(MotionE&0x10))&&(!(MotionE&0x08))
			#if 0
...
{
	#if 0
	// Configure the motion line as input, interrupt enabled
	gpio->configurePin(Paw3205MOTION_PORT, Paw3205MOTION_PIN,
					GpioDriver::INPUT_ENABLE |
					GpioDriver::OUTPUT_DISABLE |
					GpioDriver::EN_INT_LEVEL_LOW|
					GpioDriver::PULL_UP,
					GpioDriver::PIN_OUTPUT_HIGH);
	
	// Clear any spurious interrupts
	gpio->clearPortInterruptStatus(Paw3205MOTION_PORT, 1 << Paw3205MOTION_PIN);
	#endif
	//Full chip reset	
