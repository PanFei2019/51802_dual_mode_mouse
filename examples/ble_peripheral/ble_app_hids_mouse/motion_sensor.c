...
#define KA5857_REG_VALUE_PRODUCT_ID1                    0x58
#define KA5857_REG_VALUE_PRODUCT_ID2                    0x57
...
{	
	...
	#endif
	if(value==Paw_3205_REG_VALUE_PRODUCT_ID1||value==Paw_3065_REG_VALUE_PRODUCT_ID1||value==KA5857_REG_VALUE_PRODUCT_ID1)
	{
...
void turnOn(void)
{
	...
	writeReg(0x09,0x5A);
	//writeReg(0x05,0xB9);	 
	writeReg(0x0A,0xF0);	 //  Sleep 1, frequency 64ms, time 32ms
	writeReg(0x0B,0x30);	 //  Sleep 2, frequency  64*4=256ms, time 20.48s
	writeReg(0x0C,0xF0);	 //  Slepp 3, frequency 1024ms, time 20.48s
	...
	else if(ChipVersion==0x57)
	{
		#if DEBUG_TRACE_ENABLE
		NRF_LOG_INFO("ChipSet= KA5857\r\n");
		#endif
		ChipSet = ChipSet_KA5857;
		//setCPIvalue = readReg(0x0D);
		//NRF_LOG_INFO("CPI_X:%02x\r\n",setCPIvalue);
		//KA5857
		#if DefaultHighDPI
		...
		writeReg(0x0E , 32);    //Y 1200
		#endif
		writeReg(0x69 , 0x23);    //internal working voltage
		writeReg(0x6A , 0xE0);    //sensor working voltage
	}
	else
	{
		#if 0
		...
		#else
		#if DEBUG_TRACE_ENABLE
		NRF_LOG_RAW_INFO("sensor init failed\r\n");
		#endif
		while(1)
		{	
			app_sched_execute();
...
void usrPaw3205setCPI(uint8_t CPIValue)
{
	...
	switch(CPIValue)
	{
		case 1:
			...
			else if(ChipVersion==0x02||ChipVersion==0x57)                        //PAW3212, KA5857
			{
		...
		case 4:
			...
			NRF_LOG_RAW_INFO("4\r\n");
			#endif
			if(ChipVersion==0x02||ChipVersion==0x57)                        //PAW3212, KA5857
			{
